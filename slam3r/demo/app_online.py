    ...
    while frame is not None:
        if frame_num % fps == 0:    
            ...
            if i < (initial_winsize - 1)*keyframe_stride:
                ...
                buffering_set_ids, init_ref_id, init_num = initial_scene_for_accumulated_frames(input_views, initial_winsize, keyframe_stride,
                                                     i2p_model, per_frame_res, registered_confs_mean,
                                                     args, conf_thres_i2p)
                local_confs_mean_up2now = recover_points_in_initial_window(i, buffering_set_ids, keyframe_stride,
                                                 init_ref_id, per_frame_res, win_r,
                                                 keyframe_stride, input_views, i2p_model,
                                                 conf_thres_i2p)
                # Special treatment: register the frames within the range of initial window with L2W model
                if keyframe_stride > 1:
                    max_conf_mean = register_initial_window_frames(init_num, keyframe_stride, buffering_set_ids,
            ...
            milestone, candi_frame_id = update_buffer_set(next_register_id, update_buffer_intv, max_buffer_size, keyframe_stride,
